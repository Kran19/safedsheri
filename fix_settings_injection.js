...
if (content.includes(injectedBlock)) {
  content = content.replace(injectedBlock, '');
}

// Inject it into the correct place
const targetPoint = `{activeTab === 'settings' && (
        <div className="space-y-6 animate-fade-in">`;
        
if (content.includes(targetPoint)) {
  content = content.replace(targetPoint, targetPoint + injectedBlock);
} else {
